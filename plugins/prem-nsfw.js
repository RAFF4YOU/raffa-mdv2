...
let handler  = async (m, { conn, command, args, usedPrefix, DevMode, isPrems }) => {
	...
	// ------- OTHER ------
  if (global.db.data.chats[m.chat].nsfw == true && m.isGroup) return conn.sendButton(m.chat, '❗ 𝐨𝐩𝐭𝐢𝐨𝐧𝐬 𝐍𝐒𝐅𝐖 𝐝𝐢 𝐜𝐡𝐚𝐭 𝐢𝐧𝐢 𝐛𝐞𝐥𝐮𝐦 𝐝𝐢 𝐧𝐲𝐚𝐥𝐚𝐤𝐚𝐧 𝐨𝐥𝐞𝐡 𝐚𝐝𝐦𝐢𝐧 𝐠𝐫𝐨𝐮𝐩',botdate, null, [['ᴇɴᴀʙʟᴇ', '.on nsfw']], m)
...
let next = 'ɴ ᴇ x ᴛ'
let fot = botdate
let txtprem = '❗ 𝐍𝐒𝐅𝐖 𝐢𝐧𝐢 𝐤𝐡𝐮𝐬𝐮𝐬 𝐮𝐬𝐞𝐫 𝐩𝐫𝐞𝐦𝐢𝐮𝐦\n𝐡𝐚𝐫𝐚𝐩 𝐡𝐮𝐛𝐮𝐧𝐠𝐢 𝐨𝐰𝐧𝐞𝐫 𝐮𝐧𝐭𝐮𝐤 𝐦𝐞𝐦𝐛𝐞𝐥𝐢 𝐩𝐫𝐞𝐦𝐢𝐮! 📞'
let p = '🅟 | '
let f = 'Ⓕ | '

let tekk = `\`\`\`➩ Random Image Nsfw ${args[0] ? args[0].capitalize() : true}\`\`\` `
